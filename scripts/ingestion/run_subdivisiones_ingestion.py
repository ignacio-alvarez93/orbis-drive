...
from src.localizacion.id_resolution.id_resolution_localizacion import IDResolutionLocalizacion
from src.localizacion.loaders.localizacion_loader import LocalizacionLoader
from src.localizacion.normalization.normalizador_subdivisiones import NormalizadorSubdivisiones
from src.localizacion.validacion_lote.lote_localizacion import LoteLocalizacionValidator


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ejecuta el pipeline completo e ingesta subdivisiones administrativas."
    )
...
def default_report_path(csv_path: Path) -> Path:
    return Path("data/samples/output") / f"{csv_path.stem}__ingestion_subdivisiones_report.json"
...
def main() -> int:
    ...
    try:
        ...
        ingestion_result = loader.ingest_subdivisiones(
            rows=resolved_rows,

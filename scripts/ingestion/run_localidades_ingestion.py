...
from src.localizacion.normalization.normalizador_localidades import NormalizadorLocalidades
from src.localizacion.validacion_lote.lote_localizacion import LoteLocalizacionValidator


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ejecuta el pipeline completo e ingesta T_Localidades en SQLite."
    )
...
def default_report_path(csv_path: Path) -> Path:
    return Path("data/samples/output") / f"{csv_path.stem}__ingestion_localidades_report.json"
...
def main() -> int:
    ...
    try:
        ...
        ingestion_result = loader.ingest_localidades(
            rows=resolved_rows,

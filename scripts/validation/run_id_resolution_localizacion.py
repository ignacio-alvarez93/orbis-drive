...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ejecuta IIG + NORMALIZACION + DVL + VALIDACION_LOTE + ID_RESOLUTION para T_Paises."
    )
    parser.add_argument(
        "--contract",
        required=True,
...
def default_report_path(csv_path: Path) -> Path:
    return Path("data/samples/output") / f"{csv_path.stem}__id_resolution_paises_report.json"
...
def print_summary(payload: dict[str, Any]) -> None:
    ...
    idr = payload["id_resolution_summary"]
    ...
    if not dvl["is_valid_batch"]:
        print("[WARN] DVL falló. No se ejecutan capas posteriores.")
        return
    ...
    if not lote["is_valid_dataset"]:
        print("[WARN] VALIDACION_LOTE falló. No se ejecuta ID_RESOLUTION.")
        return

    print(f"[INFO] ID_RESOLUTION total rows: {idr['total_rows']}")
    print(f"[INFO] ID_RESOLUTION valid rows: {idr['valid_rows']}")
    print(f"[INFO] ID_RESOLUTION invalid rows: {idr['invalid_rows']}")

    if idr["is_valid_batch"]:
        print("[OK] ID_RESOLUTION_Localizacion completada: lote persistible.")
    else:
        print("[WARN] ID_RESOLUTION_Localizacion detectó incidencias.")
        for item in idr["invalid_rows_detail"][:10]:
...
def build_payload(
    ...
    id_resolution_batch_result: Any,
) -> dict[str, Any]:
    return {
        "entity": id_resolution_batch_result.entity,
        "contract_path": str(contract_path),
        "csv_path": str(csv_path),
        "pipeline_stage": (
            "IIG -> NORMALIZACION_TERRITORIAL -> DVL -> "
            "VALIDACION_LOTE -> ID_RESOLUTION"
        ),
        "iig_summary": {
        ...
        "id_resolution_summary": {
            "total_rows": id_resolution_batch_result.total_rows,
            "valid_rows": id_resolution_batch_result.valid_rows,
            "invalid_rows": id_resolution_batch_result.invalid_rows,
            "is_valid_batch": id_resolution_batch_result.is_valid_batch,
            "invalid_rows_detail": [
                {
                    "row_index": row.row_index,
                    "errors": [to_jsonable(err) for err in row.errors],
                ...
                for row in id_resolution_batch_result.results
                if not row.is_valid
            ],
        },
        "resolved_records": [
            to_jsonable(row.resolved_record)
            for row in id_resolution_batch_result.results
            if row.is_valid and row.resolved_record is not None
        ],
    }
...
def main() -> int:
    ...
    try:
        ...
        if not lote_batch_result.is_valid_dataset:
            payload = {
        ...
        id_resolver = IDResolutionLocalizacion()
        id_resolution_batch_result = id_resolver.resolve_rows(lote_valid_rows)
        ...
        payload = build_payload(
            ...
            id_resolution_batch_result=id_resolution_batch_result,
        )

        print_summary(payload)
        write_report(report_path, payload)

        return 0 if id_resolution_batch_result.is_valid_batch else 1

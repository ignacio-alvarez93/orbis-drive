...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ejecuta IIG + NORMALIZACION_TERRITORIAL para T_Paises."
    )
    parser.add_argument(
        "--contract",
        required=True,
...
def default_report_path(csv_path: Path) -> Path:
    return Path("data/samples/output") / f"{csv_path.stem}__normalizacion_paises_report.json"
...
def print_summary(payload: dict[str, Any]) -> None:
    ...
    if norm["is_valid_batch"]:
        print("[OK] Normalización completada: lote normalizado correctamente.")
    else:
        print("[WARN] La normalización detectó incidencias.")
...
def main() -> int:
    ...
    try:
        ...
        normalization_batch_result = normalizer.normalize_rows(valid_rows)

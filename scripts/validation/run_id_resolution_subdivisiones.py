...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Ejecuta IIG + NORMALIZACION + DVL + VALIDACION_LOTE + "
            "ID_RESOLUTION para T_Subdivisiones_Administrativas."
        )
    )
    ...
    parser.add_argument("--csv", required=True, dest="csv_path", help="Ruta al CSV fuente.")
...
def default_report_path(csv_path: Path) -> Path:
    return Path("data/samples/output") / f"{csv_path.stem}__id_resolution_subdivisiones_report.json"
...
def print_summary(payload: dict[str, Any]) -> None:
    ...
    if idr["is_valid_batch"]:
        print("[OK] ID_RESOLUTION de subdivisiones completada: lote persistible.")
    else:
        print("[WARN] ID_RESOLUTION de subdivisiones detectó incidencias.")
...
def main() -> int:
    ...
    try:
        ...
        idr = IDResolutionLocalizacion(db_path=db_path)
        idr_result = idr.resolve_rows(lote_valid_rows)

        payload = {
            "entity": idr_result.entity,
            "db_path": str(db_path),
            ...
            "id_resolution_summary": {
                "total_rows": idr_result.total_rows,
                "valid_rows": idr_result.valid_rows,
                "invalid_rows": idr_result.invalid_rows,
                "is_valid_batch": idr_result.is_valid_batch,
                "invalid_rows_detail": [
                    {
                        "row_index": rr.row_index,
                        "errors": [to_jsonable(err) for err in rr.errors],
            ...
            "resolved_records": [
                to_jsonable(rr.resolved_record)
                for rr in idr_result.results
        ...
        return 0 if idr_result.is_valid_batch else 1

...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ejecuta IIG + NORMALIZACION_TERRITORIAL para T_Localidades."
    )
    parser.add_argument(
        "--contract",
        required=True,
        help="Ruta al contrato JSON de localidades.",
    )
    parser.add_argument(
        "--csv",
        required=True,
        dest="csv_path",
        help="Ruta al CSV fuente de localidades.",
    )
    parser.add_argument(
        "--report",
        required=False,
...
def default_report_path(csv_path: Path) -> Path:
    return Path("data/samples/output") / f"{csv_path.stem}__normalizacion_localidades_report.json"
...
def build_payload(
    ...
    normalized_records = [
        to_jsonable(row.normalized_record)
        for row in normalization_batch_result.results
        if row.is_valid and row.normalized_record is not None
    ]
...
def print_summary(payload: dict[str, Any]) -> None:
    ...
    if norm["is_valid_batch"]:
        print("[OK] Normalización de localidades completada correctamente.")
    else:
        print("[WARN] La normalización de localidades detectó incidencias.")

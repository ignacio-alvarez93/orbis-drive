...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Ejecuta IIG + NORMALIZACION + DVL + VALIDACION_LOTE para T_Localidades."
    )
...
def default_report_path(csv_path: Path) -> Path:
    return Path("data/samples/output") / f"{csv_path.stem}__lote_localidades_report.json"
...
def print_summary(payload: dict[str, Any]) -> None:
    ...
    if lote["is_valid_dataset"]:
        print("[OK] VALIDACION_LOTE de localidades superada: dataset globalmente válido.")
    else:
        print("[WARN] VALIDACION_LOTE de localidades detectó conflictos.")
